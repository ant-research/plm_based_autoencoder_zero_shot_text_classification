...
from model.bertmodel.model.model import BertClassifierModel
...
class OneModelTrainer_BERT(Trainbasic):
    '''
    训练模型
    '''
    def __init__(self, model_config, logger, dataloader_list, label_mat, generated_dataset, emb_layer=None, local=False, **kwargs):
        ...
        self.unseen_class = [i for i in self.seen_test if i not in self.seen_train]
        print('unseen class number is', self.unseen_class)
        print('seen class number is', self.seen_train)
    ...
    def build_model(self):
        ...
        initialize model
        '''
        self.model = BertClassifierModel(model_config=self.model_config, emb_layer=self.emb_layer, label_matrix=self.label_mat)
...
class OneModelTrainerParallel_Bert(OneModelTrainer_BERT):
    def __init__(self, model_config, logger, dataloader_list, label_mat, generated_dataset, emb_layer=None, local=False, **kwargs):
