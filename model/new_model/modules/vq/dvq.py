...
class DVQ(nn.Module):
    def __init__(
        self, num_embeddings, embedding_dim, config, decompose_option="slice", **kwargs
    ):
        super().__init__()
    ...
    def quantize_embedding(self, indice: torch.Tensor) -> torch.Tensor:
        ...
        for vq_layer in self.vq_layers:
            quantized = vq_layer.quantize_embedding(indice[:, i])  # type: ignore
            i += 1
