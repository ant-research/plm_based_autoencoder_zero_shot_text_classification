...
if getattr(torch, "bool", None) is not None:
    dtype_memory_size_dict[torch.bool] = 8/8  
...
class MemTracker(object):
    ...
    """
    def __init__(self, detail=True, path='./log/', verbose=False, device=0):
        ...
        with open(self.gpu_profile_fn, 'a+') as f:
            ...
            if self.print_detail is True:
                ...
                for t, s, n, m, data_type in new_tensor_sizes - self.last_tensor_sizes:
                    print(f'+ | {str(n)} * Size:{str(s):<20} | Memory: {str(m*n)[:6]} M | {str(t):<20} | {data_type}\n')
