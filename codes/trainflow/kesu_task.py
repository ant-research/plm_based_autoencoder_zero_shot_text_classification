...
from codes.kesu.dataloader import (
    ...
    KESU_Idx_Entailment_Dataset,
    KESU_Generated_Enailment_Dataset
    )
...
class KESUFlow(TaskFlow):
    ...
    def preprocess_data(self):
        ...
        for data_type in ['train', 'eval', 'test']:
            ...
            if self.pre_idx:
                if self.entailment:
                    total_dataset = KESU_Idx_Entailment_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
                    print('total dataset length is', len(total_dataset))               
                else:
                    total_dataset = KESU_Idx_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
            else:
                total_dataset = KESU_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                        emb_layer=self.emb_layer, init=init, local=local, debug=debug)
        ...
        if self.entailment:
            self.generate_dataset = KESU_Generated_Enailment_Dataset(class_num=self.model_config.class_num,
        ...
        label_dataset = KESU_Idx_Label_dataset(self.data_config, data_type='train', logger=self.logger, label=True,
                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
