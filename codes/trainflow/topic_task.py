...
from codes.topic.dataloader import (
    Topic_Dataset,
    Topic_Idx_Dataset,
    Topic_Generated_Dataset,
    Topic_Unlabeled_Dataset,
    Topic_Idx_Unlabeled_Dataset,
    Topic_Idx_Label_dataset,
    Topic_Idx_Entailment_Dataset,
    Topic_Generated_Enailment_Dataset
    )
from codes.topic.emb import EmbeddingLayer
# import model config
from codes.topic.model_config import ModelConfig
from codes.topic.dataset_config import TopicDataConfigDICT, TopicDataConfig_Local
from model import ModelTrainer, ModelTrainerParallel
from model.one_trainer import OneTaskTrainer #, OneTaskTrainerBERT

class TopicFlow(TaskFlow):
    def __init__(self, model_dict, dataset_dict, emb_type='w2v', parallel=False, local=True, model='New', debug=False,
        ...
        if self.emb_type == 'bert':
            self.data_config = TopicDataConfigDICT['bert_t'](emb_type=self.emb_type, **dataset_dict)
            self.data_bert_p_config = TopicDataConfigDICT['bert_p'](emb_type=self.emb_type, **dataset_dict)
        elif self.emb_type == 'w2v':
            self.data_config = TopicDataConfigDICT['w2v'](emb_type=self.emb_type, **dataset_dict)
        else:
            raise KeyError
        ...
        if self.decoder_gpt == True:
            self.gpt_config = TopicDataConfigDICT['gpt2'](emb_type=self.emb_type, **dataset_dict)
    ...
    def preprocess_data(self):
        ...
        for data_type in ['train', 'test', 'eval']:
            ...
            if self.pre_idx:
                if self.entailment:
                    total_dataset = Topic_Idx_Entailment_Dataset(self.data_config, data_type, logger=self.logger, label=True,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)                    
                else:
                    total_dataset = Topic_Idx_Dataset(self.data_config, data_type, logger=self.logger, label=True,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
            else:
                total_dataset = Topic_Dataset(self.data_config, data_type, logger=self.logger, label=True,
                                                        emb_layer=self.emb_layer, init=init, local=local, debug=debug)
        ...
        if self.entailment:
            self.generate_dataset = Topic_Generated_Enailment_Dataset(class_num=self.model_config.class_num,
                                                                      dataset_config=self.data_config)
        else:
            self.generate_dataset = Topic_Generated_Dataset(class_num=self.model_config.class_num,
                                                            dataset_config=self.data_config)
        # load unlabel dataset
        self.logger.info('end total dataset build')
        if self.pre_idx:
            self.unlabel_dataset = Topic_Idx_Unlabeled_Dataset(dataset_config=self.data_config, init=init,
            ...
            self.unlabel_dataset = Topic_Unlabeled_Dataset(dataset_config=self.data_config, init=init,
                                                          emb_layer=self.emb_layer, debug=debug, local=local)
        ...
        label_dataset = Topic_Idx_Label_dataset(self.data_config, data_type='eval', logger=self.logger, label=True,
                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
