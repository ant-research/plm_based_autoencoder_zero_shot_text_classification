...
from codes.situation.dataloader import (
    Situation_Dataset,
    Situation_Idx_Dataset,
    Situation_Generated_Dataset,
    Situation_Unlabeled_Dataset,
    Situation_Idx_Unlabeled_Dataset,
    Situation_Idx_Label_dataset,
    Situation_Idx_Entailment_Dataset,
    Situation_Generated_Enailment_Dataset
    )
from codes.situation.emb import EmbeddingLayer
# import model config
from codes.situation.model_config import ModelConfig
from codes.situation.dataset_config import SituationDataConfigDICT, SituationDataConfig_Local
from model import ModelTrainer, ModelTrainerParallel
from model.one_trainer import OneTaskTrainer #, OneTaskTrainerBERT

class SituationFlow(TaskFlow):
    def __init__(self, model_dict, dataset_dict, emb_type='w2v', parallel=False, local=True, model='New', debug=False,
        ...
        if self.emb_type == 'bert':
            self.data_config = SituationDataConfigDICT['bert_t'](emb_type=self.emb_type, **dataset_dict)
            self.data_bert_p_config = SituationDataConfigDICT['bert_p'](emb_type=self.emb_type, **dataset_dict)
        elif self.emb_type == 'w2v':
            self.data_config = SituationDataConfigDICT['w2v'](emb_type=self.emb_type, **dataset_dict)
        else:
            raise KeyError
        ...
        if self.decoder_gpt == True:
            self.gpt_config = SituationDataConfigDICT['gpt2'](emb_type=self.emb_type, **dataset_dict)
    ...
    def preprocess_data(self):
        ...
        for data_type in ['train', 'test', 'eval']:
            ...
            if self.pre_idx:
                if self.entailment:
                    total_dataset = Situation_Idx_Entailment_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)                    
                else:
                    total_dataset = Situation_Idx_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
            else:
                total_dataset = Situation_Dataset(self.data_config, data_type, logger=self.logger, label=label,
                                                        emb_layer=self.emb_layer, init=init, local=local, debug=debug)
        ...
        if self.entailment:
            self.generate_dataset = Situation_Generated_Enailment_Dataset(class_num=self.model_config.class_num,
                                                                      dataset_config=self.data_config)
        else:
            self.generate_dataset = Situation_Generated_Dataset(class_num=self.model_config.class_num,
                                                            dataset_config=self.data_config)
        # load unlabel dataset
        self.logger.info('end total dataset build')
        if self.pre_idx:
            self.unlabel_dataset = Situation_Idx_Unlabeled_Dataset(dataset_config=self.data_config, init=init,
            ...
            self.unlabel_dataset = Situation_Unlabeled_Dataset(dataset_config=self.data_config, init=init,
                                                          emb_layer=self.emb_layer, debug=debug, local=local)
        ...
        label_dataset = Situation_Idx_Label_dataset(self.data_config, data_type='eval', logger=self.logger, label=True,
                                            emb_layer=self.emb_layer, init=init, local=local, debug=debug)
