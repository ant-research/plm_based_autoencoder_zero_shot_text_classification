...
class Topic_Dataset(Dataset):
    ...
    def get_y_data(self, y_data):
        ...
        '''
        total_codes = list(self.label_csv['id'])
        self.logger.info('total code labels: %d' % len(total_codes))
        self.y_data = []
        seen_codes_list = []
        for label in y_data:
            index = label # total_codes.index(label)
...
class Topic_Idx_Dataset(Topic_Dataset):
    """
    dataset类
    """
    def __init__(self, dataset_config, data_type, emb_layer, logger, label=False, init=False, debug=False, local=True):
...
class Topic_Idx_Entailment_Dataset(Topic_Dataset):
    """
    dataset类
    """
    def __init__(self, dataset_config, data_type, emb_layer, logger, label=False, init=False, debug=False, local=True):
    ...
    def deal_x_data(self, df, y_data):
        ...
        # def get_batched_data(input_ids: list, label_length: int, batch_number: int):
        #     input_ids = [[input_ids[i+j] for j in range(label_length)] for i in range(batch_number)]
        #     return input_ids
...
class Topic_Idx_Label_dataset(Topic_Dataset):
    '''
    ...
    def __init__(self, dataset_config, data_type, emb_layer, logger, label=False, init=True, debug=False, local=True):
        ...
        self.x_da = list(label_csv['da_description'])
...
class Topic_Generated_Dataset(Dataset):
    '''
    generated class and result
    '''
...
class Topic_Generated_Enailment_Dataset(Dataset):
    '''
    generated class and result
    '''
    def __init__(self, class_num, dataset_config):
        ...
        self.class_num = class_num
        label_csv = pd.read_csv(dataset_config.label_local_path)
        self.y_text = ['hypothesis of ' + str(label_name) for label_name in list(label_csv['label'])]
...
class Topic_Unlabeled_Dataset(Dataset):
    """
    dataset类
    """
...
class Topic_Idx_Unlabeled_Dataset(Topic_Unlabeled_Dataset):
    """
    dataset类
    """
    def __init__(self, dataset_config, emb_layer, debug=False, local=False, init=False):
