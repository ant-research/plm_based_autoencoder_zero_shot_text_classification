...
class W2VEmbedding(object):
    ...
    def tokenize(self, sentence, get_cls=False, unk=False):
        ...
        self.cutter.load_userdict('./data/kesu/keyword_list.txt')
        sentence = sentence.replace(' ', '') # 去掉空格
        words = self.cutter.lcut(sentence)
        ...
        for word in words:
            ...
            if word in self.vocab_dict.keys():
                ...
                for little_word in new_cut:
                    if little_word in self.vocab_dict.keys():
                        new_list.append(little_word)
                        input_list.append(self.vocab_dict[little_word])
                        padding_mask.append(1)
                        count += 1
                    new_word_list = re.split("|".join(new_list), little_word)
                    for word_l3 in new_word_list:
                        if word_l3 in self.vocab_dict.keys():
                            input_list.append(self.vocab_dict[word_l3])
                            padding_mask.append(1)
                            count += 1
                        elif unk:
        ...
        assert count != 0, 'No word in this sentence'
